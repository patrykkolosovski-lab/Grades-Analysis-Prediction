...
public class FileDisplayerSample1 {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while (elementScanner.hasNextLine() && linesDone < 50) {
            	...
            	while (lineScanner.hasNext()) {
                    if (lineScanner.hasNext("NG")) {
                        GraduateGradesArray[linesDone-1][j] = 0;
                        lineScanner.next();
                        j++;
			...
			for (int i = 0; i < Findhighest(GraduateGradesArray).length; i++) {
				System.out.println(Findhighest(GraduateGradesArray)[i]);
			}
